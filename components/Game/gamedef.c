...
/* Skip whitespace only (not // comments - those are handled as keyword 20)
   Returns 1 if more text available, 0 if end of text */
int skipwhitespace(void)
{
    while(*textptr == ' ' || *textptr == 0x0d || *textptr == 0x0a || *textptr == '\t')
    {
        if(*textptr == 0x0a) line_number++;
        textptr++;
        if(*textptr == 0) return 0;
    }
    return 1;  /* More text available */
}
...
keyword_restart:
    ...
    {
        if(*temptextptr == 0x0a) ; /* don't count lines in peek function */
        temptextptr++;
        if( *temptextptr == 0 )
            return 0;
    }
    ...
    if (*temptextptr == '/' && *(temptextptr+1) == '/')
    {
        while (*temptextptr != 0x0a && *temptextptr != 0)
            temptextptr++;
        if (*temptextptr == 0)
            return 0;
        goto keyword_restart;
    }
...
transword_restart:
    ...
    /* Check for // comment (with or without space after) */
    if (*textptr == '/' && *(textptr+1) == '/')
    {
        /* Skip to end of line */
        while (*textptr != 0x0a && *textptr != 0)
            textptr++;
        goto transword_restart;
    }
...
transnum_restart:
    ...
    /* Skip // comments - jump to next line and restart */
    if (*textptr == '/' && *(textptr+1) == '/')
    {
        while (*textptr != 0x0a && *textptr != 0)
            textptr++;
        goto transnum_restart;
    }
