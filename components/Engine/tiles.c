...

{
    ...
    /* Cap cache size to leave room for video buffers (at least 512KB) */
    cachesize = max(artsize,1048576);
    #ifdef RP2350_PSRAM
    /* Limit cache to 1.5MB max to leave room for video and other allocations */
    if (cachesize > 1536*1024) {
        cachesize = 1536*1024;
        printf("Capping cache size to %d bytes for RP2350\n", cachesize);
    }
    #endif
